...
function AddServiceImg() {
  ...
  const handleFileUpdate = (files) => {
    setData((prevData) => ({
      ...prevData,
      serviceImage: files.length > 0 ? files[0].file : null,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/admin/AddServiceImg`,
        formData
      );
      if (response.data.success) {
        ...
        toast.error("Serviceimg Not Added!!", {
          autoClose: 1500,
        });
      }
    } catch (error) {
